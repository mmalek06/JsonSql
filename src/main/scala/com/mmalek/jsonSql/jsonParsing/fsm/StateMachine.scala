...
class StateMachine(val state: State) {
  ...
  // no double quotes here, as " is a marker for a new scalar
  private val specialChars = List(32, 33) ++ (35 to 47).toList ++ (58 to 64).toList ++ (91 to 96).toList ++ (123 to 126).toList
  ...
  private def canReadArrayAfterScalar(c: Char, valueSoFar: String, history: Seq[State]) =
    if (c == '[' && isIn(ReadArray, ReadArrayEnd, history)) Some(ReadArray) else None

  private def canReadScalarAfterScalar(c: Char, valueSoFar: String, history: Seq[State]) =
    if (c != ' ' && valueSoFar.trim.lastOption.contains(',') && isScalarReadFullyInArray(valueSoFar.trim.last, valueSoFar, history)) Some(ReadScalar)
    else None
  ...
  private def canReadArrayEndHistorically(c: Char, valueSoFar: String, history: Seq[State]) =
  ...
  private def isScalarReadFullyInArray(c: Char, valueSoFar: String, history: Seq[State]) =
    ...
    else isScalarReadFully(c, valueSoFar)
