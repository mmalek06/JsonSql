...
class TokenizerTests extends AnyFlatSpec with Matchers {
  "A Tokenizer" should "parse simple input into tokens" in {
    val query = """SELECT "thing" FROM ##json## WHERE "some.thing" = 1"""
  ...
  it should "parse input with one wildcard symbol and no fields into tokens" in {
    ...
    tokens.head should be (Select)
    tokens(1) should be (Field("*"))
    tokens(2) should be (From)
  }

  it should "parse input with wildcards into tokens" in {
    val query =
      """
        SELECT "items.address.*", "items.id"
        FROM ##json##
      """.stripMargin
    val (tokens, error) = Tokenizer.tokenize(query)
    ...
    tokens.head should be (Select)
    tokens(1) should be (Field("items.address.*"))
    tokens(2) should be (Field("items.id"))
    tokens(3) should be (From)
  }

  it should "parse math expressions into tokens" in {
    val query =
      """
        SELECT 2 * 3, "items.age"
        FROM ##json##
        """
    val (tokens, error) = Tokenizer.tokenize(query)
    ...
    tokens.head should be (Select)
    tokens(1) should be (Constant("2"))
    tokens(2) should be (Operator("*"))
    tokens(3) should be (Constant("3"))
    tokens(4) should be (Field("items.age"))
    tokens(5) should be (From)
  }
